			       ...
			       dma_addr_t dma_addr, u32 chan)
{
	...
	/* PDMA assignment; optionally set AXI outstanding read queue depth */
			       ...
			       dma_addr_t dma_addr, u32 chan)
{
	...
	/* PDMA assignment; enable RX PDMA; optionally set AXI outstanding write queue depth */
				      ...
				      struct stmmac_dma_cfg *dma_cfg, u32 chan)
{
	...
	u32 pdma = (dma_cfg->tx_vdma_pdma_map && chan < dma_cfg->total_tx_vdma) ?
		   dma_cfg->tx_vdma_pdma_map[chan] : chan % priv->plat->tx_queues_to_use;
	u32 tc = (dma_cfg->tx_vdma_tc_map && chan < dma_cfg->total_tx_vdma) ?
		 dma_cfg->tx_vdma_tc_map[chan] : chan % priv->plat->tx_queues_to_use;
	u32 value;
				      ...
				      struct stmmac_dma_cfg *dma_cfg, u32 chan)
{
	...
	u32 pdma = (dma_cfg->rx_vdma_pdma_map && chan < dma_cfg->total_rx_vdma) ?
		   dma_cfg->rx_vdma_pdma_map[chan] : chan % priv->plat->rx_queues_to_use;
	u32 tc = (dma_cfg->rx_vdma_tc_map && chan < dma_cfg->total_rx_vdma) ?
		 dma_cfg->rx_vdma_tc_map[chan] : chan % priv->plat->rx_queues_to_use;
	u32 value;
